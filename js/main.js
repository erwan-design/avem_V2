            ...
            function hideMain() {
                ...
                $('html, body').animate({ scrollTop: 0 }, 300, function() {
                    ...
                    function loadContent() {
                        $('#main').load(link + ' #main > *', function(c) {
    ...
    if ($('#portfolio-container').length > 0) {
        ...
        if (!isMobile && $('.title-tooltip').length > 0 ) {
            ...
            $(".portfolio-item").on('mouseleave', function() { 
                $circleCursor.css('background-color', '#47CDA2');
...
//Flexnav Menu
$(".flexnav").flexNav({ 'animationSpeed': 200 });
...
function HeroSection() {
    ...
    if ($('#hero').length > 0) {
        ...
        window.addEventListener('scroll', function windowScroll() {
            ...
            Hero.style.opacity = (1 - (windowScrolled / 30) / 10);
...
function contact() {
    ...
    if ($('#contactForm').length > 0) {
        ...
        function submitForm() { var name = $("#name").val(); var email = $("#email").val(); var msg_subject = $("#msg_subject").val(); var message = $("#message").val();
            $.ajax({ type: "POST", url: "php/form-process.php", data: "name=" + name + "&email=" + email + "&msg_subject=" + msg_subject + "&message=" + message, success: function(text) { if (text == "success") { formSuccess(); } else { formError();
